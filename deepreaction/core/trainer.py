...
class ReactionTrainer:
    ...
    def _validate_config(self):
        ...
        if len(self.config.dataset.target_fields) != len(self.config.dataset.input_features):
            self.logger.warning(f"Target fields ({len(self.config.dataset.target_fields)}) and input features ({len(self.config.dataset.input_features)}) count mismatch")
        
        if len(self.config.dataset.target_weights) != len(self.config.dataset.target_fields):
            self.logger.warning(f"Target weights length ({len(self.config.dataset.target_weights)}) doesn't match target fields ({len(self.config.dataset.target_fields)})")
    ...
    def _create_dataloader(self, dataset, batch_size, eval_mode=False):
        ...
        self.logger.info(f"Creating dataloader: batch_size={batch_size}, num_workers={num_workers}, eval_mode={eval_mode}")
    ...
    def _setup_callbacks(self, output_dir):
        ...
        if self.config.training.early_stopping_patience > 0:
            ...
            self.logger.info(f"Added early stopping callback (patience: {self.config.training.early_stopping_patience})")
    ...
    def fit(self, train_dataset, val_dataset, test_dataset=None, scalers=None, checkpoint_path=None, mode='train') -> Dict[str, Any]:
