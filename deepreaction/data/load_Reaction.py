...
def create_data_split(dataset, indices, ensure_2d=True):
    ...
    if failed_indices:
        logger = logging.getLogger('deepreaction')
        logger.warning(
            f"Failed to load {len(failed_indices)} samples: {failed_indices[:10]}{'...' if len(failed_indices) > 10 else ''}")

        if len(failed_indices) > len(indices) * 0.1:
            raise RuntimeError(
                f"Too many failed samples ({len(failed_indices)}/{len(indices)}). Data may be corrupted.")
