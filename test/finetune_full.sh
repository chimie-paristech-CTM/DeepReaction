#!/bin/bash
# full_finetune.sh
# This script performs full fine-tuning of all model layers
# to completely adapt the pre-trained model to the reaction dataset
...
OUTPUT_DIR="${SCRIPT_DIR}/results/finetune/full"

# Default parameters
DATASET="XTB"
TARGET_ID=0
BATCH_SIZE=16
EPOCHS=100  # More epochs for full fine-tuning
MIN_EPOCHS=5
EARLY_STOPPING=15
...
WEIGHT_DECAY=0.0001
DROPOUT=0.1
GRADIENT_ACCUMULATION=2  # Accumulate gradients for larger effective batch size
MAX_NUM_ATOMS=100
...
CMD="${CMD} --save_best_model --save_last_model --save_interval 10 --save_predictions --save_visualizations"
CMD="${CMD} --progress_bar"
CMD="${CMD} --cuda"
CMD="${CMD} --precision 16"  # Use mixed precision for faster training
CMD="${CMD} --random_seed 42"
# No freeze_backbone flag - all layers will be fine-tuned
...
# Print command
echo "========================================================================================="
echo "Running FULL FINE-TUNING on reaction dataset:"
echo "- Dataset:       XTB"
...
echo "- Strategy:      Full fine-tuning (all layers)"
