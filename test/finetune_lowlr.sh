#!/bin/bash
# low_lr_finetune.sh
# This script performs fine-tuning with a very low learning rate
# to carefully adapt the pre-trained model to the reaction dataset
...
OUTPUT_DIR="${SCRIPT_DIR}/results/finetune/low_lr"

# Default parameters
DATASET="XTB"
TARGET_ID=0
BATCH_SIZE=16
EPOCHS=50  # More epochs for low learning rate
MIN_EPOCHS=5
EARLY_STOPPING=20
...
# Print command
echo "========================================================================================="
echo "Running fine-tuning with LOW LEARNING RATE on reaction dataset:"
echo "- Dataset:       XTB"
echo "- Pretrained model: ${MODEL_PATH}"
echo "- Batch size:    ${BATCH_SIZE}"
echo "- Epochs:        ${EPOCHS} (min: ${MIN_EPOCHS})"
echo "- Learning rate: ${LR} (very low for careful adaptation)"
echo "- Optimizer:     ${OPTIMIZER}"
echo "- Scheduler:     ${SCHEDULER} (warmup: ${WARMUP_EPOCHS})"
echo "- Strategy:      Low learning rate fine-tuning (entire model)"
...
if [ $? -eq 0 ]; then
    echo "========================================================================================="
    echo "Fine-tuning with low learning rate completed successfully!"
