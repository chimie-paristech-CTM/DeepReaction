...
def plot_training_curves(metrics_file, output_dir, save_format='png'):
    ...
    if 'train_loss' in metrics_df.columns and 'val_loss' in metrics_df.columns:
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(epochs, metrics_df['train_loss'], label='Training Loss')
        ax.plot(epochs, metrics_df['val_loss'], label='Validation Loss')
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
    ...
    for metric in metric_types:
        ...
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, f'{metric.lower()}_curve.{save_format}'), dpi=300)
        plt.close(fig)
