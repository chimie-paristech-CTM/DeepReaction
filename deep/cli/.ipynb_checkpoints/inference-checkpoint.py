...
def create_inference_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dataset", type=str, default=None,
                        choices=[None, "QM7", "QM8", "QM9", "QMugs", "XTB", "benzene", "aspirin", "malonaldehyde", "ethanol", "toluene"],
...
def load_dataset(args, logger):
    ...
    if args.dataset == 'QM7':
        train, val, test, scaler = load_QM7(42, args.target_id)
    elif args.dataset == 'QM8':
        train, val, test, scaler = load_QM8(42, args.target_id)
        ...
        train, val, test, scaler = load_QMugs(42, args.target_id)
...
def run_inference(model, dataloader, use_cuda, scaler=None, uncertainty=False, mc_samples=10):
    """
...
def run_ensemble_inference(models, dataloader, use_cuda, scaler=None):
    """
    ...
    for i, model in enumerate(models):
        ...
        # Run inference
        result = run_inference(model, dataloader, use_cuda, scaler)
...
def main():
    ...
    # Set the scaler on the model(s) if needed
    if args.use_scaler:
        if args.ensemble:
            for model in models:
                model.scaler = scaler
    ...
    for split_name, dataloader in dataloaders.items():
        ...
        try:
            if args.ensemble:
                ...
                results = run_ensemble_inference(models, dataloader, args.cuda, scaler)
