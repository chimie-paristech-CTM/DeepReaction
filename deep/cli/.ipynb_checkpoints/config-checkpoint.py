...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.ensemble_size > 1 and args.ensemble_seeds is not None:
        if len(args.ensemble_seeds) != args.ensemble_size:
            raise ValueError(f"Number of ensemble seeds ({len(args.ensemble_seeds)}) must match ensemble size ({args.ensemble_size})")
    ...
    if args.max_epochs < args.min_epochs:
        raise ValueError(f"Maximum epochs ({args.max_epochs}) must be greater than or equal to minimum epochs ({args.min_epochs})")
