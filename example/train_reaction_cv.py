...
def get_parser():
    parser = argparse.ArgumentParser(description='Train a molecular reaction prediction model with 10-fold cross-validation')
...
def main():
    ...
    for fold in range(dataset.get_num_folds()):
        ...
        trainer = ReactionTrainer(
            ...
            save_best_model=args.save_best_model,
            save_last_model=args.save_last_model,
            random_seed=args.random_seed + fold,  # Different seed for each fold
        ...
        print(f"Fold {fold+1} training completed: {train_metrics}")
        if 'best_model_path' in train_metrics:
            print(f"Fold {fold+1} best model saved to: {train_metrics['best_model_path']}")
        ...
        if trainer.trainer is not None and dataset.test_data is not None and len(dataset.test_data) > 0:
            ...
            print(f"Fold {fold+1} test metrics: {test_results}")
            
            if test_results and isinstance(test_results, list) and len(test_results) > 0:
                test_metrics = test_results[0]
                
                # Get test loss
                if 'test_total_loss' in test_metrics:
                    cv_test_losses.append(test_metrics['test_total_loss'])
        
        # Save fold results
        fold_results = {
            'fold': fold,
            'train_metrics': train_metrics,
            'test_metrics': test_results[0] if test_results and len(test_results) > 0 else None
        }
        cv_metrics.append(fold_results)
